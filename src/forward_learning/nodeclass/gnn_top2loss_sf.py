...
class GNNTop2LossSFLayer(torch.nn.Module):
    def __init__(
        self,
        gnn_layer,
        optimizer_name: str,
    ...
    def forward_train(
        ...
        input_state = states[layer_index]
        cur_layer_output_state = self.forward(
            x=input_state.detach(),
            edge_index=edge_index,
            edge_type=edge_type
        )
        above_layer_output_states = states[layer_index + 2:]
        cur_and_above_states = [cur_layer_output_state] + \
                               [state.detach() for state in above_layer_output_states]
        node_emb = torch.mean(torch.stack(cur_and_above_states), dim=0)
        ...
        return cur_layer_output_state, loss.item()


class GNNTop2LossSF(torch.nn.Module):
    def __init__(
        self,
        layer_sizes: list,
        optimizer_name: str,
        ...
        self.states = []
        self.layer_sizes = layer_sizes  # [num-inputs, hid-dim-1, hid-dim-2, ..., hid-dim-H]
        self.layers = torch.nn.ModuleList()

        for i in range(0, len(layer_sizes) - 1):
            if self.append_label == "input" and i == 0:
                in_channels = num_classes + layer_sizes[i]
            else:
    ...
    def bottom_up(
        ...
        x_proj = x
        states = [self.norm(x_proj)]

        for layer_i, layer in enumerate(self.layers):
            x_proj = layer.forward(x=x_proj, edge_index=edge_index, edge_type=edge_type)
            states.append(self.norm(x_proj).detach())
        ...
        try:
            for time_step in range(self.time_steps):
                ...
                for j, layer in enumerate(self.layers):
                    ...
                    new_states[j + 1] = self.norm(node_emb).detach()
                    ts_loss += loss
    ...
    def eval_model(
        ...
        for layer_index, layer in enumerate(self.layers):
            ...
            cur_layer_output_state = layer.forward(
                x=input_state,
                edge_index=aug_graph.edge_index,
                edge_type=aug_graph.edge_type
            )
            ...
            """compute class prob"""
            eval_node_feats = node_emb[self.graph_augmenter.real_node_mask][eval_mask]
            virtual_node_feats = node_emb[~self.graph_augmenter.real_node_mask]
            ...
            if layer_index in storable_layers:
                accumulated_probs.append(prob)
